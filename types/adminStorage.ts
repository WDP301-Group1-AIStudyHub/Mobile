...
export interface AdminPaymentTransaction { id: string; orderRef: string; status: AdminPaymentStatus; provider: AdminPaymentProvider; clientPlatform: AdminPaymentPlatform; amountVnd: number; currency: string; packageId: string; package: AdminTransactionPackageSnapshot; providerTxnRef: string; providerResponseCode: string; bankCode: string; settledBy: string | null; failureReason: string; completedAt: string | null; expiresAt: string; createdAt: string; updatedAt: string; user: AdminUserSummary | null }

...
const WELCOME_MSG: Message = {
  ...
  role: "assistant",
  content:
    "Hello! I am your AI study assistant. Choose a subject or document first, then ask about your study material.",
  time: "",
};
...
export default function ChatIndexPage() {
  ...
  const filteredDocs = useMemo(() => {
    ...
    return docs.filter(
      (doc) =>
        matchesQuery(doc.title, q) || matchesQuery(doc.fileName || "", q),
    );
  }, [docs, searchQuery]);
  const semesterGroups = useMemo(
    () => groupDocsBySemester(filteredDocs),
    [filteredDocs],
  );
      ...
      setExpandedSubjects(
        new Set(
          groups.flatMap((group) =>
            group.subjects.map((subject) => `${group.label}::${subject.key}`),
          ),
        ),
      );
    } catch {
  ...
  useEffect(() => {
    if (
      documentId &&
      documentId !== handledDocIdRef.current &&
      docsById.has(documentId) &&
      !inChatMode
    ) {
        ...
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              setThreads((prev) =>
                prev.filter((t) => (t.id || t._id) !== threadId),
              );
  ...
  const openSavedThread = async (thread: ChatThreadItem) => {
    ...
    try {
      ...
      if (!isMountedRef.current) return;
      setSelectedDocIds(
        detail.thread.documentIds?.length
          ? detail.thread.documentIds
          : detail.thread.documentId
            ? [detail.thread.documentId]
            : [],
      );
      const mapped: Message[] = detail.messages.flatMap((msg) => [
        {
          id: `${msg._id || msg.id}-user`,
          role: "user",
      ...
      Alert.alert(
        "Load Thread Failed",
        err.message || "Could not load conversation.",
      );
    } finally {
      if (isMountedRef.current) setHistLoading(false);
    }
  ...
  const toggleSubjectSelection = (subjectDocs: DocumentItem[]) => {
    ...
    const allSelected =
      ids.length > 0 && ids.every((id) => selectedDocIds.includes(id));
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      // Render the complete answer immediately
      setMessages((prev) => [
        ...prev,
        {
          id: answerId,
    ...
    return (
      <View style={[S.msgRow, isUser ? S.msgRowUser : S.msgRowAi]}>
        ...
        <View style={[S.bubble, isUser ? S.bubbleUser : S.bubbleAi]}>
          <ChatMarkdown isUser={isUser}>{item.content}</ChatMarkdown>
          {item.time ? (
            ...
            <View style={S.flex}>
              ...
              <View style={S.tabsContainer}>
                <Pressable
                  style={[
                    S.tabButton,
                    activeTab === "context" && S.tabButtonActive,
                  ]}
                ...
                >
                  <Text
                    style={[
                      S.tabButtonText,
                      activeTab === "context" && S.tabButtonTextActive,
                    ]}
                  >
                    Study Context
                  </Text>
                ...
                <Pressable
                  style={[
                    S.tabButton,
                    activeTab === "history" && S.tabButtonActive,
                  ]}
                ...
                >
                  <Text
                    style={[
                      S.tabButtonText,
                      activeTab === "history" && S.tabButtonTextActive,
                    ]}
                  >
                    Chat History
                  </Text>
                ...
                <View style={S.flex}>
                  {docsLoading ? (
                    ...
                    <ScrollView
                      ...
                      style={[S.panelScroll, { paddingHorizontal: Spacing.md }]}
                    >
                      <Pressable
                        style={[
                          S.contextRow,
                          selectedDocIds.length === 0 && S.contextRowActive,
                        ]}
                        onPress={clearContext}
                      >
                        <Library size={17} color={C.primary} />
                        <Text style={S.contextRowText}>
                          No subject selected
                        </Text>
                        {selectedDocIds.length === 0 ? (
                          <Check size={17} color={C.primary} />
                        ) : null}
                      </Pressable>

                      {semesterGroups.map((semester) => {
                        const semesterOpen = expandedSemesters.has(
                          semester.label,
                        );
                        const totalDocs = semester.subjects.reduce(
                          (total, subject) => total + subject.docs.length,
                          0,
                        );
                        return (
                          <View key={semester.label}>
                            <Pressable
                              style={S.semHeader}
                              onPress={() => toggleSemester(semester.label)}
                            >
                            ...
                            {semesterOpen &&
                              semester.subjects.map((subject) => {
                                ...
                                const subjectOpen =
                                  expandedSubjects.has(subjectOpenKey);
                                const allSelected =
                                  selectedSubjectKey === subject.key &&
                                  subject.docs.length > 0 &&
                                  subject.docs.every((doc) =>
                                ...
                                return (
                                  ...
                                  >
                                    <View style={S.subjectHeader}>
                                      <Pressable
                                        onPress={() =>
                                          toggleSubjectOpen(subjectOpenKey)
                                        }
                                        style={{
                                          flex: 1,
                                          flexDirection: "row",
                                      ...
                                      >
                                        <BookOpen
                                          size={15}
                                          color={subject.color || C.primary}
                                        />
                                        <Text
                                          style={S.subjectTitle}
                                          numberOfLines={1}
                                        >
                                          {subject.subject}
                                        </Text>
                                      ...
                                      <Pressable
                                        style={[
                                          S.allChip,
                                          allSelected && S.allChipActive,
                                        ]}
                                        onPress={() =>
                                          toggleSubjectSelection(subject.docs)
                                        }
                                      >
                                        <View
                                          style={[
                                            S.checkBox,
                                            allSelected && S.checkBoxActive,
                                          ]}
                                        >
                                          {allSelected ? (
                                        ...
                                        return (
                                          ...
                                          >
                                            ...
                                            <View style={S.docRowInfo}>
                                              ...
                                              >
                                                {doc.isShared
                                                  ? `${doc.accessRole === "EDITOR" ? "Editor" : "Viewer"} · `
                                                  : "Owner · "}
                                                {doc.fileName}
                                              </Text>
                                            </View>
              ...
              <View style={S.header}>
                ...
                <View style={S.headerCenter}>
                  ...
                  >
                    ...
                    <Text style={S.contextButtonText} numberOfLines={1}>
              ...
              <View style={S.inputArea}>
                <View style={S.inputBar}>
                  ...
                  <TextInput
                    ...
                    placeholder={
                      hasStudyContext
                        ? "Ask AI about your study material..."
